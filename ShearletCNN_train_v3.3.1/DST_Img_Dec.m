...
%load data111
addpath(genpath(cd));
X = imread('Test\fewBSDS500_test\35049.jpg');
scales_DST = 1;
...
X = im2double(X);
[coeffs,shearletSystem] = DSTImgDec(X,scales_DST);

figure(1);
imshow(abs([coeffs(:,:,1),coeffs(:,:,2),coeffs(:,:,3);...
    coeffs(:,:,4), coeffs(:,:,5),coeffs(:,:,6);...
    coeffs(:,:,7),coeffs(:,:,8),zeros(size(coeffs(:,:,8)))]),[]);
